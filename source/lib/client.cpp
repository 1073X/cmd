...
client::client(std::string_view server) {
    ...
    if (!_sock) {
        log::error(_name, +"failed to connect");
    }
}
...
bool client::send(uint32_t seq_num, com::strcat const& cmd, std::vector<com::variant> const& args) {
    ...
    if (_sock.send(req, req->size()) != (int32_t)req->size()) {
        log::error(_name, +"SEND", seq_num, cmd.str(), args);
        return false;
    }

    log::info(_name, +"SEND", seq_num, cmd.str(), args);
    return true;
}
...
message const* client::recv(char* buf, uint32_t len) {
    ...
    if (recv_len < (int32_t)sizeof(message)) {
        log::error(_name, +"RECV timeout");
        return nullptr;
    }

    auto msg = (message*)buf;
    if (len - sizeof(message) < msg->payload_size()) {
        return nullptr;
    }
    ...
    log::info(_name, +"RECV", msg->seq_num(), std::string(msg->cmd()), args);
